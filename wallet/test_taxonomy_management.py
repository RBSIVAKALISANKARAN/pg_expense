...
class TaxonomyManagementTests(TestCase):
    def setUp(self):
        ...
        self.main_meal = SubCategory.objects.create(
            category=self.food, name="Main Meal"
        )
        self.beverage = SubCategory.objects.create(category=self.food, name="Beverage")
        self.dosa = Item.objects.create(
            category=self.food, subcategory=self.main_meal, name="Dosa"
        )
        self.tea = Item.objects.create(
            category=self.food, subcategory=self.beverage, name="Tea"
        )
    ...
    def test_food_taxonomy_lists_fixed_dimensions_and_profiles(self):
        ...
        self.assertIn(
            {"value": "drink", "label": "Drink"}, response.data["choices"]["food_type"]
        )
        self.assertIn(
            {"value": "junk", "label": "Junk"},
            response.data["choices"]["health_classification"],
        )
