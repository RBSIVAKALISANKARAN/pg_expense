...
class Phase4SearchAndSqlTests(TestCase):
    ...
    def test_transaction_search_and_filters(self):
        response = self.client.get(
            "/api/transactions/all/?search=Phase4%20Cafe&allocation=spendable"
        )
    ...
    def test_transaction_filter_options(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(
            any(x["id"] == str(self.account.id) for x in response.data["accounts"])
        )
        self.assertTrue(
            any(x["id"] == str(self.category.id) for x in response.data["categories"])
        )
    ...
    def test_saved_query_rejects_write_sql(self):
        self.client.post(
            "/api/sql/saved/",
            {"name": "unsafe", "sql": "DROP TABLE wallet_account;"},
            format="json",
        )
        response = self.client.post(
            "/api/sql/saved/",
            {"name": "unsafe", "sql": "DELETE FROM wallet_account"},
            format="json",
        )
    ...
    def test_saved_query_accepts_read_only_sql(self):
        response = self.client.post(
            "/api/sql/saved/",
            {"name": "accounts", "sql": "SELECT id, name FROM wallet_account"},
            format="json",
        )
