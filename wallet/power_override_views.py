...
from .views import (_account_context, _assert_account_reconciles,
                    _ensure_allocations, _ensure_money_pool)
...
def _authorized(username, password):
    return hmac.compare_digest(
        str(username or ""), POWER_OVERRIDE_USERNAME
    ) and hmac.compare_digest(str(password or ""), POWER_OVERRIDE_PASSWORD)
...
def power_override(request):
    ...
    if not _authorized(data.get("username"), data.get("password")):
        return Response(
            {"detail": "Power override credentials are invalid."},
    ...
    with transaction.atomic():
        if action == "set_rbsankaran_balance":
            try:
                total = Decimal(str(data.get("total_balance", ""))).quantize(
                    Decimal("0.01")
                )
                savings = Decimal(str(data.get("savings_balance", "0"))).quantize(
                    Decimal("0.01")
                )
            except (InvalidOperation, ValueError):
                return Response(
                    {"detail": "Enter valid monetary values."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            if total < 0 or savings < 0:
                return Response(
                    {"detail": "Balances cannot be negative."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            if savings > total:
                return Response(
                    {"detail": "Savings cannot exceed total balance."},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            account = (
                Account.objects.select_for_update()
                .filter(name=TARGET_ACCOUNT_NAME)
                .first()
            )
            if account is None:
                return Response(
                    {"detail": f"Account {TARGET_ACCOUNT_NAME} was not found."},
        ...
        if action == "reset_all_balances":
            ...
            for account in accounts:
                ...
                MoneyPool.objects.filter(account=account).update(
    ...
    return Response(
        {"detail": "Unknown power override action."}, status=status.HTTP_400_BAD_REQUEST
    )
