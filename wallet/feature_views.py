...
def meals(request):
    ...
    if not name:
        return Response(
            {"detail": "Meal name is required."}, status=status.HTTP_400_BAD_REQUEST
        )
    meal, created = MealOption.objects.get_or_create(
    ...
    return Response(
        {"id": meal.id, "name": meal.name, "active": meal.active},
