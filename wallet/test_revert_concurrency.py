...
class ConcurrentTransactionRevertTests(TransactionTestCase):
    ...
    def test_opposite_transfer_reverts_do_not_deadlock(self):
        ...
        self.assertFalse(any(thread.is_alive() for thread in threads), 'Concurrent revert did not complete.')
        ...
        self.assertEqual(
            Transaction.objects.filter(type=TransactionType.TRANSFER, metadata__reverted=True).count(),
            4,
        )
