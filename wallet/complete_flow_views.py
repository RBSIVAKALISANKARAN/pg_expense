...
def _lock_two_accounts(source_id, destination_id):
    ...
    locked = {
        str(a.id): a for a in Account.objects.select_for_update().filter(id__in=ids)
    }
    if len(locked) != 2:
...
def create_wallet_account(request):
    ...
    location_type = str(
        request.data.get("location_type") or MoneyLocationType.BANK
    ).strip()
    ...
    with transaction.atomic():
        ...
        if account is None:
            account = Account.objects.create(
                name=name, currency=currency, money_location=location
            )
        else:
...
def wallet_transfer(request):
    ...
    with transaction.atomic():
        ...
        out_tx = Transaction.objects.create(
            account=source,
            owner=source_owner,
            money_location=source_location,
        ...
        in_tx = Transaction.objects.create(
            ...
            money_location=destination_location,
            allocation=destination_alloc,
    ...
    return Response(
        {
            "outgoing": TransactionSerializer(out_tx).data,
            "incoming": TransactionSerializer(in_tx).data,
        },
...
def wallet_expense_entry(request):
    ...
    try:
        ...
        category = (
            Category.objects.get(pk=data.get("category"))
            if data.get("category")
            else None
        )
        subcategory = (
            SubCategory.objects.get(pk=data.get("subcategory"))
        ...
        raise ValidationError(
            {"category": "Selected category, subcategory, or item does not exist."}
        )
    ...
    if allocation_type not in AllocationType.values:
        raise ValidationError(
            {"allocation": "Allocation must be spendable or savings."}
        )
    with transaction.atomic():
        ...
        if food_items:
            ...
            if not meal:
                raise ValidationError(
                    {"meal": "Meal is required when food items are supplied."}
                )
            event = FoodEvent.objects.create(transaction=tx, meal=meal)
            for entry in food_items:
                food_item = (
                    Item.objects.filter(pk=entry.get("item")).first()
                    if entry.get("item")
                    else None
                )
                ...
                if not food_item and not custom_name:
                    raise ValidationError(
                        {"food_items": "Each food entry needs an item or custom name."}
                    )
                quantity = Decimal(str(entry.get("quantity") or "1"))
                if quantity <= 0:
                    raise ValidationError(
                        {"food_items": "Quantity must be greater than zero."}
                    )
...
def wallet_edit_expense(request, id):
    with transaction.atomic():
        tx = (
            ...
            .filter(pk=id, type=TransactionType.EXPENSE)
            .first()
        )
        if not tx:
        ...
        subcategory = (
            SubCategory.objects.get(pk=request.data["subcategory"])
            if request.data.get("subcategory")
            else tx.subcategory
        )
        item = (
            Item.objects.get(pk=request.data["item"])
            if request.data.get("item")
            else tx.item
        )
        _validate_taxonomy(category, subcategory, item)
        (
            _validate_transport(request.data, category, location.location_type)
            if category and category.name.lower() == "transport"
            else None
        )
...
def wallet_revert_transaction(request, id):
    with transaction.atomic():
        ...
        related = (
            Transaction.objects.select_for_update().filter(pk=tx.related_tx_id).first()
            if tx.related_tx_id
            else None
        )
        ...
        for target in targets:
            ...
            if target.type == TransactionType.DEPOSIT:
                ...
                and target.metadata.get("direction") == "in"
            ):
                if not allocation or allocation.balance < amount:
                    raise ValidationError(
                        (
        ...
        for target in targets:
            ...
            if target.type == TransactionType.EXPENSE:
                ...
                destination = Allocation.objects.select_for_update().get(
                    account=account, type=destination_type
                )
...
def money_report_data(request):
    qs = (
        Transaction.objects.select_related(
            "category", "subcategory", "item", "account", "money_location"
        )
        .filter(type=TransactionType.EXPENSE)
        .exclude(metadata__reverted=True)
        .exclude(metadata__deleted=True)
    )
    result = {
        key: {}
        for key in (
    ...
    for tx in qs:
        ...
        labels = {
            ...
            "item": (
                tx.item.name
                if tx.item
                else (tx.metadata.get("custom_description") or "Custom / Other")
            ),
    ...
    with connection.cursor() as cursor:
        ...
        for name in table_names:
            ...
            for column in connection.introspection.get_table_description(cursor, name):
                try:
                    type_name = connection.introspection.get_field_type(
                        column.type_code, column
                    )
                except Exception:
                    type_name = str(column.type_code)
                columns.append(
                    {"name": column.name, "type": type_name, "nullable": column.null_ok}
                )
