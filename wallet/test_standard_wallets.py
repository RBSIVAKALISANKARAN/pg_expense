...
class StandardWalletSetupTests(TestCase):
    def test_standard_wallets_are_available_after_migrations(self):
        ...
        for name, location_type in expected.items():
            ...
            self.assertTrue(
                Allocation.objects.filter(account=account, type="spendable").exists()
            )
            self.assertTrue(
                Allocation.objects.filter(account=account, type="savings").exists()
            )
    ...
    def test_old_tmb_location_is_not_used_for_the_standard_bank_wallet(self):
        ...
        self.assertTrue(
            MoneyLocation.objects.filter(
                name="rbsankaran_acc", location_type="bank"
            ).exists()
        )
