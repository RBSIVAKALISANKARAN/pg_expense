...
@override_settings(
    TESTING=False, SQL_PLAYGROUND_TIMEOUT_MS=5000, SQL_PLAYGROUND_MAX_ROWS=500
)
class SQLPlaygroundSecurityTests(TestCase):
    def setUp(self):
        ...
        self.client.login(
            username="sql-security-user", password="StrongTestPassword123!"
        )
    ...
    def test_write_keywords_are_rejected(self):
        response = self.client.post(
            "/api/sql/execute/",
            data={"sql": "UPDATE wallet_account SET name = 'x'"},
            content_type="application/json",
        )
    ...
    def test_result_set_is_capped(self):
        response = self.client.post(
            "/api/sql/execute/",
            data={"sql": "SELECT generate_series(1, 501) AS value"},
            content_type="application/json",
        )
        self.assertEqual(response.status_code, 200)
