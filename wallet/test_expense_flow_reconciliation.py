...
class ExpenseFlowReconciliationTests(TestCase):
    def setUp(self):
        ...
        Allocation.objects.get_or_create(
            account=self.account, type=AllocationType.SPENDABLE
        )
        Allocation.objects.get_or_create(
            account=self.account, type=AllocationType.SAVINGS
        )
    ...
    def pool_total(self):
        return sum(
            MoneyPool.objects.filter(account=self.account).values_list(
                "current_amount", flat=True
            )
        )
    ...
    def test_expense_rejects_location_different_from_account_location_without_mutating(
        ...
        self.assertFalse(
            MoneyPool.objects.filter(account=self.account, location=cash).exists()
        )
