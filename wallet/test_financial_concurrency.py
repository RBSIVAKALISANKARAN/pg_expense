...
class ConcurrentFinancialOperationTests(TransactionTestCase):
    ...
    def test_two_concurrent_expenses_cannot_overspend_one_account(self):
        ...
        self.assertFalse(
            ...
            "Concurrent operation did not complete.",
        )
        ...
        self.assertEqual(
            Transaction.objects.filter(
                account=account, type=TransactionType.EXPENSE
            ).count(),
            1,
        )
    ...
    def test_opposite_concurrent_wallet_transfers_do_not_deadlock_or_overdraw(self):
        ...
        self.assertFalse(
            any(thread.is_alive() for thread in threads),
            "Concurrent transfer did not complete.",
        )
        ...
        self.assertEqual(
            Transaction.objects.filter(type=TransactionType.TRANSFER).count(), 4
        )
