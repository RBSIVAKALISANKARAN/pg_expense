...
def phase4_transaction_list(request):
    ...
    if search:
        query = (
            ...
            | Q(money_location__name__icontains=search)
            | Q(category__name__icontains=search)
            | Q(subcategory__name__icontains=search)
            | Q(item__name__icontains=search)
            | Q(variant__icontains=search)
            | Q(metadata__merchant__icontains=search)
            | Q(metadata__note__icontains=search)
            | Q(metadata__custom_description__icontains=search)
...
def phase4_transaction_filter_options(request):
    ...
    return Response(
        {
            "accounts": [
                {"id": str(x.id), "name": x.name} for x in Account.objects.all()
            ],
            "owners": [
                {"id": str(x.id), "name": x.name}
                for x in Owner.objects.filter(active=True)
            ],
            "types": [
            ...
            "categories": [
                {"id": str(x.id), "name": x.name} for x in Category.objects.all()
            ],
            "subcategories": [
                {"id": str(x.id), "name": x.name, "category": str(x.category_id)}
                for x in SubCategory.objects.select_related("category").all()
            ],
            "items": [
                {
                    "id": str(x.id),
                    "name": x.name,
                    "category": str(x.category_id),
                    "subcategory": str(x.subcategory_id) if x.subcategory_id else "",
                }
                for x in Item.objects.all()
            ],
            "allocations": [
                {"value": value, "label": label}
                for value, label in AllocationType.choices
            ],
        }
    )
    ...
    data = [
        {
            ...
            "query": x.query,
            "status": x.status,
            "execution_time_ms": x.execution_time_ms,
...
def phase4_saved_queries(request, id=None):
    ...
    name, sql, description = (
        str(request.data.get("name") or "Untitled query").strip(),
        str(request.data.get("sql") or "").strip(),
