...
class CompleteMoneyFlowTests(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="complete-flow-user", password="test-password", is_staff=True
        )
        ...
        response = self.client.post(
            f"/api/accounts/{self.upi.id}/deposit/",
            {"amount": "1000", "allocate_to_savings": "0"},
            format="json",
        )
    ...
    def test_transfer_moves_money_between_wallets_without_duplication(self):
        ...
        self.assertEqual(
            self.balance(self.upi) + self.balance(self.travel), Decimal("1000")
        )
