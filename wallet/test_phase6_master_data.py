...
class Phase6MasterDataTests(TestCase):
    ...
    def test_accounts_can_be_created_and_updated(self):
        location = MoneyLocation.objects.create(
            name="Phase6 Bank", location_type="bank"
        )
        response = self.client.post(
            reverse("master-accounts"),
            {
                "name": "Phase6 Account",
                "currency": "INR",
                "money_location": str(location.id),
            },
        ...
        response = self.client.post(
            reverse("master-account-detail", args=[account.id]),
    ...
    def test_non_zero_account_cannot_be_archived(self):
        account = Account.objects.create(
            name="Phase6 Nonzero", total_balance=Decimal("100")
        )
        response = self.client.post(
            reverse("master-account-status", args=[account.id]),
            {"active": False},
            content_type="application/json",
        )
    ...
    def test_category_archive_cascades_to_subcategories_and_items(self):
        ...
        item = Item.objects.create(
            category=category, subcategory=sub, name="Phase6 Item"
        )
        response = self.client.post(
            reverse("master-category-status", args=[category.id]),
            {"active": False},
            content_type="application/json",
        )
        self.assertEqual(response.status_code, 200, response.content)
    ...
    def test_subcategory_archive_cascades_to_items(self):
        ...
        item = Item.objects.create(
            category=category, subcategory=sub, name="Phase6 Item 2"
        )
        response = self.client.post(
            reverse("master-subcategory-status", args=[sub.id]),
            {"active": False},
            content_type="application/json",
        )
        self.assertEqual(response.status_code, 200, response.content)
    ...
    def test_master_hierarchy_rejects_inactive_parent(self):
        ...
        response = self.client.post(
            reverse("master-subcategories"),
            {"category": str(category.id), "name": "Blocked"},
            content_type="application/json",
        )
    ...
    def test_owner_can_be_archived_and_reactivated(self):
        ...
        response = self.client.post(
            reverse("master-owner-status", args=[owner.id]),
            {"active": False},
            content_type="application/json",
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_money_location_used_by_active_account_cannot_be_archived(self):
        location = MoneyLocation.objects.create(
            name="Phase6 Used Location", location_type="bank"
        )
        Account.objects.create(name="Phase6 Location Account", money_location=location)
        response = self.client.post(
            reverse("master-location-status", args=[location.id]),
            {"active": False},
            content_type="application/json",
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_configuration_accepts_active_defaults(self):
        ...
        location = MoneyLocation.objects.create(
            name="Phase6 Active Location", location_type="bank"
        )
        response = self.client.post(
            reverse("master-config"),
            {
                "default_owner": owner.name,
                "default_money_location": location.name,
