...
SQL_PLAYGROUND_TIMEOUT_MS = max(
    100, int(os.getenv("SQL_PLAYGROUND_TIMEOUT_MS", "5000"))
)
...
def _validate_sql(raw_sql):
    ...
    if not sql:
        raise ValueError("SQL query is required.")
    if any(
        re.search(pattern, sql, re.IGNORECASE) for pattern in FORBIDDEN_SQL_PATTERNS
    ):
...
def sql_execute_secure(request):
    """Execute bounded, read-only SQL for staff users only."""
    ...
    try:
        ...
        return Response(
            {"status": "error", "message": str(exc), "execution_time_ms": 0},
    ...
    try:
        ...
        with transaction.atomic():
            with connection.cursor() as cursor:
                ...
                cursor.execute(
                    "SET LOCAL statement_timeout = %s", [SQL_PLAYGROUND_TIMEOUT_MS]
                )
                cursor.execute(sql)
                columns = (
        ...
        QueryExecutionLog.objects.create(
            query=sql, status="success", execution_time_ms=execution_time_ms
        )
        return Response(
            {
                "status": "success",
                ...
                + (
                    " Result set truncated at the configured row limit."
                    if truncated
                    else ""
                ),
                "columns": columns,
