...
class PowerOverrideTests(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="power-test-user", password="test-password"
        )
        ...
        self.target.save(
            update_fields=["money_location", "total_balance", "updated_at"]
        )
        self.other = Account.objects.create(
            name="Power Test Wallet", total_balance=Decimal("75")
        )
    ...
    def post_override(self, action, **extra):
        ...
        return self.client.post(
            reverse("power-override"), payload, content_type="application/json"
        )
    ...
    def test_set_rbsankaran_balance_reconciles_allocations_and_pools(self):
        ...
        self.assertEqual(
            Allocation.objects.get(
                account=self.target, type=AllocationType.SPENDABLE
            ).balance,
            Decimal("375.00"),
        )
        self.assertEqual(
            Allocation.objects.get(
                account=self.target, type=AllocationType.SAVINGS
            ).balance,
            Decimal("125.00"),
        )
        self.assertEqual(
            MoneyPool.objects.filter(account=self.target).aggregate(
    ...
    def test_reset_all_balances_sets_accounts_allocations_and_pools_to_zero(self):
        ...
        self.assertEqual(
            Account.objects.get(pk=self.target.pk).total_balance, Decimal("0.00")
        )
        self.assertEqual(
            Account.objects.get(pk=self.other.pk).total_balance, Decimal("0.00")
        )
    ...
    def test_invalid_override_credentials_are_rejected(self):
        ...
        self.assertEqual(
            Account.objects.get(pk=self.target.pk).total_balance, Decimal("250.00")
        )
