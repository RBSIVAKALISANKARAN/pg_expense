...
class ExpenseWalletFeatureTests(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="feature-workflow-user", password="test-password", is_staff=True
        )
        self.client.force_login(self.user)
        self.owner = Owner.objects.get(name="Me")
        self.location = MoneyLocation.objects.create(
            name="UPI Wallet", location_type="bank"
        )
        self.travel = MoneyLocation.objects.get(name="Travel Card")
        self.account = Account.objects.create(
            name="UPI", currency="INR", money_location=self.location
        )
    ...
    def test_sql_playground_is_read_only(self):
        response = self.client.post(
            "/api/sql/execute-live/", {"sql": "SELECT 1"}, format="json"
        )
        self.assertEqual(response.status_code, 200)
        response = self.client.post(
            "/api/sql/execute-live/",
            {"sql": "DELETE FROM wallet_transaction"},
            format="json",
        )
