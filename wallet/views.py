...
def account_list_create(request):
    ...
    with transaction.atomic():
        ...
        if requested_location is None:
            requested_location, _ = MoneyLocation.objects.get_or_create(name=serializer.validated_data['name'], defaults={'location_type': 'bank', 'active': True})
...
def allocate_funds(request, id):
    ...
    serializer.is_valid(raise_exception=True)
    amount = serializer.validated_data['amount']; source_type = serializer.validated_data['from_type']; target_type = serializer.validated_data['to_type']
    with transaction.atomic():
        account = Account.objects.select_for_update().get(id=id)
        _ensure_allocations(account)
        ...
        source.balance = F('balance') - amount; target.balance = F('balance') + amount
        ...
        Transaction.objects.create(account=account, owner=owner, money_location=money_location, allocation=source, source_pool=source_pool, type=TransactionType.ALLOCATION, amount=amount, metadata={'from': source_type, 'to': target_type})
        _assert_account_reconciles(account)
    return Response(AccountSerializer(account).data)
...
def expense_create(request, id):
    ...
    with transaction.atomic():
        ...
        try:
            ...
            return Response({'detail': str(exc.detail[0]) if isinstance(exc.detail, list) else str(exc.detail)}, status=status.HTTP_400_BAD_REQUEST)
        allocation.balance = F('balance') - amount; account.total_balance = F('total_balance') - amount
        ...
        category = serializer.validated_data.get('category'); subcategory = serializer.validated_data.get('subcategory'); item = serializer.validated_data.get('item'); meal = serializer.validated_data.get('meal'); variant = serializer.validated_data.get('variant', '')
        expense_tx = Transaction.objects.create(account=account, owner=owner, money_location=money_location, allocation=allocation, source_pool=source_pool, category=category, subcategory=subcategory, item=item, variant=variant, meal=meal, type=TransactionType.EXPENSE, amount=amount, metadata={'merchant': serializer.validated_data.get('merchant', ''), 'note': serializer.validated_data.get('note', ''), 'custom_description': serializer.validated_data.get('custom_description', '')})
        ...
        if food_items:
            ...
            for entry in food_items:
                item_id = entry.get('item'); custom_name = (entry.get('custom_name') or '').strip(); variant = entry.get('variant', ''); quantity = entry.get('quantity', 1)
                food_item = None
                if item_id:
                    try:
                ...
                FoodEventItem.objects.create(event=food_event, item=food_item, custom_name=custom_name, variant=variant, quantity=quantity)
        _assert_account_reconciles(account)
    return Response(AccountSerializer(account).data)
...
def transfer_to_savings(request, id):
    ...
    with transaction.atomic():
        ...
        try:
            ...
            return Response({'detail': str(exc.detail[0]) if isinstance(exc.detail, list) else str(exc.detail)}, status=status.HTTP_400_BAD_REQUEST)
        spendable.balance = F('balance') - amount; savings.balance = F('balance') + amount
        spendable.save(update_fields=['balance']); savings.save(update_fields=['balance'])
        spendable.refresh_from_db(); savings.refresh_from_db()
        ...
        Transaction.objects.create(account=account, owner=owner, money_location=money_location, allocation=savings, source_pool=source_pool, type=TransactionType.TRANSFER, amount=amount, metadata={'direction': 'to_savings'})
        _assert_account_reconciles(account)
    return Response(AccountSerializer(account).data)
...
def transfer_to_spendable(request, id):
    ...
    with transaction.atomic():
        ...
        try:
            ...
            return Response({'detail': str(exc.detail[0]) if isinstance(exc.detail, list) else str(exc.detail)}, status=status.HTTP_400_BAD_REQUEST)
        savings.balance = F('balance') - amount; spendable.balance = F('balance') + amount
        savings.save(update_fields=['balance']); spendable.save(update_fields=['balance'])
        savings.refresh_from_db(); spendable.refresh_from_db()
        ...
        Transaction.objects.create(account=account, owner=owner, money_location=money_location, allocation=spendable, source_pool=source_pool, type=TransactionType.TRANSFER, amount=amount, metadata={'direction': 'to_spendable'})
        _assert_account_reconciles(account)
    return Response(AccountSerializer(account).data)
...
def summary_report(request, id):
    account = get_object_or_404(Account, id=id)
    summary = summarize_account_transactions(account, request.query_params.get('start_date'), request.query_params.get('end_date'))
...
def export_report(request, id):
    account = get_object_or_404(Account, id=id)
    csv_data = export_account_csv(account, request.query_params.get('start_date'), request.query_params.get('end_date'))
...
def categories_list_create(request):
    ...
    serializer = CategorySerializer(data=request.data); serializer.is_valid(raise_exception=True); category = serializer.save()
...
def subcategories_list_create(request):
    if request.method == 'GET':
        return Response(SubCategorySerializer(SubCategory.objects.select_related('category'), many=True).data)
    category_id = request.data.get('category'); name = (request.data.get('name') or '').strip(); description = request.data.get('description') or ''
    if not category_id: return Response({'detail': 'Category is required.'}, status=status.HTTP_400_BAD_REQUEST)
    if not name: return Response({'detail': 'Subcategory name is required.'}, status=status.HTTP_400_BAD_REQUEST)
...
def items_list_create(request):
    if request.method == 'GET':
        return Response(ItemSerializer(Item.objects.select_related('category', 'subcategory'), many=True).data)
    name = (request.data.get('name') or '').strip(); category_id = request.data.get('category'); subcategory_id = request.data.get('subcategory'); description = request.data.get('description') or ''
    is_custom_raw = request.data.get('is_custom'); is_custom = str(is_custom_raw).strip().lower() in ('1', 'true', 'yes', 'on') if is_custom_raw is not None else False
    food_group = (request.data.get('food_group') or '').strip() or None; health_classification = (request.data.get('health_classification') or '').strip() or None; sugary = (request.data.get('sugary') or '').strip() or None
    if not name: return Response({'detail': 'Item name is required.'}, status=status.HTTP_400_BAD_REQUEST)
    if not category_id: return Response({'detail': 'Category is required.'}, status=status.HTTP_400_BAD_REQUEST)
    category = get_object_or_404(Category, id=category_id); subcategory = None
    if subcategory_id:
        subcategory = get_object_or_404(SubCategory, id=subcategory_id)
        if subcategory.category_id != category.id: return Response({'detail': 'Subcategory does not belong to the selected category.'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    if any(value for value in [food_group, health_classification, sugary]):
        FoodProfile.objects.update_or_create(item=item, defaults={'food_group': food_group or 'other', 'health_classification': health_classification or 'unknown', 'sugary': sugary or 'unknown'})
...
def food_profiles(request):
    if request.method == 'GET':
        return Response(FoodProfileSerializer(FoodProfile.objects.select_related('item', 'item__category'), many=True).data)
    item_id = request.data.get('item')
    if not item_id: return Response({'detail': 'Item is required.'}, status=status.HTTP_400_BAD_REQUEST)
    item = get_object_or_404(Item, id=item_id); serializer = FoodProfileSerializer(data=request.data); serializer.is_valid(raise_exception=True)
    profile, _ = FoodProfile.objects.update_or_create(item=item, defaults={'food_group': serializer.validated_data.get('food_group', 'other'), 'health_classification': serializer.validated_data.get('health_classification', 'unknown'), 'sugary': serializer.validated_data.get('sugary', 'unknown')})
    return Response(FoodProfileSerializer(profile).data, status=status.HTTP_201_CREATED)


@api_view(['GET'])
def owners_list(request):
    _ensure_family_defaults(); owners = Owner.objects.filter(active=True).order_by('name')
    return Response([{'id': str(owner.id), 'name': owner.name, 'active': owner.active} for owner in owners])


@api_view(['GET'])
def money_locations_list(request):
    _ensure_family_defaults(); locations = MoneyLocation.objects.filter(active=True).order_by('name')
    return Response([{'id': str(location.id), 'name': location.name, 'location_type': location.location_type, 'active': location.active} for location in locations])
...
def money_pools_list(request):
    ...
    return Response([{'id': str(pool.id), 'owner': {'id': str(pool.owner_id), 'name': pool.owner.name}, 'location': {'id': str(pool.location_id), 'name': pool.location.name}, 'allocation_type': pool.allocation_type, 'current_amount': str(pool.current_amount)} for pool in pools])
...
def settings_page(request):
    return render(request, 'settings.html', {'app_name': 'Expense Tracking Savings Spendable', 'currency_default': 'INR', 'timezone': 'Asia/Kolkata', 'default_allocation': 'spendable',})


@api_view(['POST'])
def sql_execute(request):
    raw_sql = request.data.get('sql', '') if isinstance(request.data, dict) else ''; start = perf_counter()
    try:
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(sql); columns = [col[0] for col in cursor.description] if cursor.description else []; rows = cursor.fetchall()
            ...
            return Response({'status': 'success', 'message': 'Query executed successfully.', 'columns': columns, 'rows': safe_rows, 'row_count': len(rows), 'execution_time_ms': execution_time_ms}, status=status.HTTP_200_OK)
    except Exception as exc:
        execution_time_ms = int((perf_counter() - start) * 1000)
        QueryExecutionLog.objects.create(query=sql if 'sql' in locals() else str(raw_sql), status='error', execution_time_ms=execution_time_ms, error_message=str(exc))
...
def sql_history(request):
    if request.method == 'GET':
        ...
        return Response([{'id': str(item.id), 'query': item.query, 'status': item.status, 'execution_time_ms': item.execution_time_ms, 'error_message': item.error_message, 'created_at': item.created_at.isoformat()} for item in logs])
...
def sql_saved_queries(request, id=None):
    if request.method == 'GET':
        ...
        return Response([{'id': str(query.id), 'name': query.name, 'description': query.description, 'sql': query.sql, 'created_at': query.created_at.isoformat()} for query in queries])
    if request.method == 'POST':
        name = request.data.get('name', '').strip() or 'Untitled query'; sql = request.data.get('sql', '').strip(); description = request.data.get('description', '').strip()
        if not sql: return Response({'detail': 'SQL is required.'}, status=status.HTTP_400_BAD_REQUEST)
        query = SavedQuery.objects.create(name=name, description=description, sql=sql)
        return Response({'id': str(query.id), 'name': query.name, 'description': query.description, 'sql': query.sql}, status=status.HTTP_201_CREATED)
    if request.method == 'DELETE':
        query = get_object_or_404(SavedQuery, id=id); query.delete(); return Response({'detail': 'Saved query deleted.'}, status=status.HTTP_200_OK)
    return Response({'detail': 'Method not allowed'}, status=status.HTTP_405_METHOD_NOT_ALLOWED)
...
def sql_schema(request):
    try:
        with connection.cursor() as cursor:
            ...
            tables = [row[0] for row in cursor.fetchall()]; schema = []
            for table_name in tables:
                ...
                columns = [{'name': column_name, 'type': data_type, 'nullable': is_nullable == 'YES'} for column_name, data_type, is_nullable in cursor.fetchall()]
...
def dashboard(request): get_token(request); return render(request, 'dashboard.html')
def accounts_page(request): get_token(request); return render(request, 'accounts.html')
def transactions_page(request): get_token(request); return render(request, 'transactions.html')
def categories_page(request): get_token(request); return render(request, 'categories.html')
def report_page(request): get_token(request); return render(request, 'reports.html')
def sql_playground(request): get_token(request); return render(request, 'sql_playground.html')
...
def database_structure_page(request):
    ...
    return render(request, 'database_structure.html', {
        'tables': [
            {'name': 'wallet_account', 'description': 'Top-level money container for a wallet/account.', 'columns': ['id', 'name', 'total_balance', 'currency', 'created_at', 'updated_at']},
            {'name': 'wallet_allocation', 'description': 'Spendable and savings split per account.', 'columns': ['id', 'account_id', 'type', 'balance', 'created_at', 'updated_at']},
            {'name': 'wallet_transaction', 'description': 'All money movements, expenses, and transfers.', 'columns': ['id', 'account_id', 'owner_id', 'money_location_id', 'allocation_id', 'source_pool_id', 'category_id', 'subcategory_id', 'item_id', 'meal', 'type', 'amount', 'metadata', 'created_at', 'related_tx_id']},
            {'name': 'wallet_category', 'description': 'Primary expense category bucket.', 'columns': ['id', 'name', 'description', 'created_at', 'updated_at']},
