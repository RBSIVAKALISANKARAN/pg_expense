...
class MoneyFlowRegressionTests(TestCase):
    def setUp(self):
        ...
        self.client.post(
            "/api/wallet/accounts/create/",
            {
                "name": "UPI",
                "location_type": "bank",
                "location_name": "Regression UPI",
            },
            format="json",
        )
        ...
        self.client.post(
            f"/api/accounts/{self.upi.id}/deposit/", {"amount": "1000"}, format="json"
        )
    ...
    def test_partial_transport_edit_preserves_existing_transport_details(self):
        ...
        response = self.client.patch(
            f"/api/transactions/{tx_id}/edit/",
            {
                "amount": "75",
                "category": str(self.transport.id),
                "transport_from": "Guindy",
                "transport_to": "T Nagar",
                "transport_mode": "bus",
                "bus_type": "MTC",
                "payment_method": "upi",
            },
        ...
        response = self.client.patch(
            f"/api/transactions/{tx_id}/edit/", {"amount": "80"}, format="json"
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_upi_to_travel_card_transfer_preserves_total_money(self):
        response = self.client.post(
            "/api/wallet/transfer/",
            {
                "source_account": str(self.upi.id),
                "destination_account": str(self.travel.id),
                "amount": "300",
            },
            format="json",
        )
        ...
        self.assertEqual(
            self.upi.total_balance + self.travel.total_balance, Decimal("1000")
        )
    ...
    def test_primary_wallet_deposit_repairs_legacy_account_only_balance(self):
        ...
        self.assertEqual(account.total_balance, Decimal("500"))
        self.assertEqual(
            account.allocations.get(type="spendable").balance, Decimal("500")
        )
        self.assertEqual(account.allocations.get(type="savings").balance, Decimal("0"))
        self.assertEqual(
            account.money_pools.filter(allocation_type="spendable").aggregate(
    ...
    def test_standard_allocation_transfer_keeps_total_balance(self):
        ...
        self.assertEqual(
            account.allocations.get(type="spendable").balance, Decimal("750")
        )
        self.assertEqual(
            account.allocations.get(type="savings").balance, Decimal("250")
        )
