...
class ConcurrentMutationConsistencyTests(TransactionTestCase):
    ...
    def test_concurrent_deposits_preserve_account_allocation_and_pool_totals(self):
        ...
        self.assertFalse(any(thread.is_alive() for thread in threads), 'Concurrent deposit did not complete.')
        ...
        pool = ensure_account_money_pool(
            account, account.money_pools.first().owner, account.money_location, AllocationType.SPENDABLE
        )
        ...
        self.assertEqual(Transaction.objects.filter(account=account, type=TransactionType.DEPOSIT).count(), 2)

    def test_concurrent_allocation_transfers_preserve_total_balance_and_pool_totals(self):
        account = self._seed_account('rbsankaran_acc', '1000.00')
        barrier = Barrier(2)
        results = []
        ...
        threads = [
            Thread(target=allocate, args=(AllocationType.SPENDABLE, AllocationType.SAVINGS)),
            Thread(target=allocate, args=(AllocationType.SAVINGS, AllocationType.SPENDABLE)),
        ]
        for thread in threads:
            thread.start()
        for thread in threads:
