...
class FinancialIntegrityTests(TestCase):
    ...
    def test_rbsankaran_acc_deposit_reconciles(self):
        ...
        self.assertEqual(
            Allocation.objects.get(
                account=self.source, type=AllocationType.SPENDABLE
            ).balance,
            Decimal("1000.00"),
        )
        self.assertEqual(
    ...
    def test_savings_and_spendable_transfers_preserve_total_balance_and_reconcile(self):
        ...
        savings = Allocation.objects.get(
            account=self.source, type=AllocationType.SAVINGS
        )
        self.source.refresh_from_db()
        ...
        pools = MoneyPool.objects.filter(
            account=self.source, owner=self.owner, location=self.location
        )
        self.assertEqual(
            pools.get(allocation_type=AllocationType.SPENDABLE).current_amount,
            Decimal("650.00"),
        )
        self.assertEqual(
            pools.get(allocation_type=AllocationType.SAVINGS).current_amount,
            Decimal("350.00"),
        )
    ...
    def test_expense_and_revert_restore_every_financial_balance(self):
        ...
        tx = Transaction.objects.get(
            account=self.source, type=TransactionType.EXPENSE, amount=Decimal("250.00")
        )
    ...
    def test_expense_cannot_overdraw_specific_account_pool(self):
        ...
        second = Account.objects.create(
            name="Pool Isolation Account", money_location=self.location, currency="INR"
        )
        ...
        self.assertEqual(
            Transaction.objects.filter(
                account=second, type=TransactionType.EXPENSE
            ).count(),
            0,
        )
