...
def master_accounts(request):
            ...
            [
                {
                    ...
                    "currency": a.currency,
                    "money_location": (
                        str(a.money_location_id) if a.money_location_id else None
                    ),
                    "location_name": (
                        a.money_location.name if a.money_location_id else None
                    ),
                    "total_balance": str(a.total_balance),
                    "active": a.active,
                }
    ...
    if Account.objects.filter(name=name).exists():
        return Response(
            {"detail": "An account with this name already exists."}, status=400
        )
    location_id = data.get("money_location")
    location = (
        get_object_or_404(MoneyLocation, id=location_id, active=True)
        if location_id
        else None
    )
...
def master_account_detail(request, pk):
    ...
    if "money_location" in data:
        location = get_object_or_404(
            MoneyLocation, pk=data.get("money_location"), active=True
        )
    ...
    return Response(
        {
            ...
            "currency": account.currency,
            "money_location": (
                str(account.money_location_id) if account.money_location_id else None
            ),
            "active": account.active,
        }
    )
...
def master_account_status(request, pk):
    ...
    if not active and account.total_balance != 0:
        return Response(
            {"detail": "An account with a non-zero balance cannot be archived."},
            status=400,
        )
...
def master_subcategories(request):
    ...
    if SubCategory.objects.filter(category=category, name=name).exists():
        return Response(
            {"detail": "This subcategory already exists in the selected category."},
            status=400,
        )
...
def master_items(request):
            ...
            [
                {
                    ...
                    "subcategory_name": (
                        i.subcategory.name if i.subcategory_id else None
                    ),
                    "name": i.name,
    ...
    if data.get("subcategory"):
        sub = get_object_or_404(
            SubCategory, pk=data.get("subcategory"), category=category, active=True
        )
    try:
        name = _name(data)
    except ValueError as exc:
    ...
    if Item.objects.filter(category=category, subcategory=sub, name=name).exists():
        return Response(
            {"detail": "This item already exists in the selected hierarchy."},
            status=400,
        )
    ...
    return Response(
        {
            "id": str(item.id),
            "category": str(category.id),
            "subcategory": str(sub.id) if sub else None,
            "name": item.name,
            "active": item.active,
        },
        status=201,
...
def master_item_detail(request, pk):
    ...
    if "category" in data:
        item.category = get_object_or_404(
            Category, pk=data.get("category"), active=True
        )
    if "subcategory" in data:
        value = data.get("subcategory")
        item.subcategory = (
            get_object_or_404(
                SubCategory, pk=value, category=item.category, active=True
            )
            if value
            else None
        )
...
def master_owners(request):
    ...
    if Owner.objects.filter(name=name).exists():
        return Response(
            {"detail": "An owner with this name already exists."}, status=400
        )
...
def master_locations(request):
    ...
    if MoneyLocation.objects.filter(name=name).exists():
        return Response(
            {"detail": "A money location with this name already exists."}, status=400
        )
...
def master_location_detail(request, pk):
    ...
    if "name" in data:
        ...
        if MoneyLocation.objects.exclude(pk=location.pk).filter(name=name).exists():
            return Response(
                {"detail": "A money location with this name already exists."},
                status=400,
            )
        location.name = name
...
def master_config(request):
    ...
    if (
        "default_owner" in data
        and not Owner.objects.filter(
            name=str(data["default_owner"]).strip(), active=True
        ).exists()
    ):
        return Response(
            {"detail": "default_owner must reference an active owner."}, status=400
        )
    if (
        "default_money_location" in data
        and not MoneyLocation.objects.filter(
            name=str(data["default_money_location"]).strip(), active=True
        ).exists()
    ):
        return Response(
            {
