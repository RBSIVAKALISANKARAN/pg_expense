...
def summarize_account_transactions(account, start_date=None, end_date=None):
    ...
    income = (
        qs.filter(type=TransactionType.DEPOSIT).aggregate(total=Sum("amount"))["total"]
        or 0
    )
    expenses = (
        qs.filter(type=TransactionType.EXPENSE).aggregate(total=Sum("amount"))["total"]
        or 0
    )
    transfers = (
        qs.filter(type=TransactionType.TRANSFER).aggregate(total=Sum("amount"))["total"]
        or 0
    )
