...
def food_taxonomy(request):
    if request.method == "GET":
        profiles = FoodProfile.objects.select_related(
            "item", "item__category", "item__subcategory"
        ).order_by("item__name")
        return Response(
            {
    ...
    if not item_id:
        return Response(
            {"detail": "Item is required."}, status=status.HTTP_400_BAD_REQUEST
        )

    try:
        item = Item.objects.select_related("category", "subcategory").get(
            id=item_id, active=True
        )
    except Item.DoesNotExist:
        return Response(
            {"detail": "Selected item was not found."}, status=status.HTTP_404_NOT_FOUND
        )
