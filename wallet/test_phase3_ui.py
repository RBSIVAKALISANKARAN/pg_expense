...
class Phase3SavingsUiContractTests(TestCase):
    def setUp(self):
        get_user_model().objects.create_user(
            username="phase3-savings", password="phase3-pass"
        )
    ...
    def test_savings_and_spendable_controls_preserve_total(self):
        ...
        response = self.client.post(
            f"/api/accounts/{self.account.id}/transfer-to-savings/",
            {"amount": "50.00"},
            format="json",
        )
        ...
        self.assertEqual(
            self.account.allocations.get(type="spendable").balance, Decimal("450.00")
        )
        self.assertEqual(
            self.account.allocations.get(type="savings").balance, Decimal("150.00")
        )
