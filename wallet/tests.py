...
class WalletTests(TestCase):
    def setUp(self):
        ...
        Allocation.objects.get_or_create(
            account=self.acc, type=AllocationType.SPENDABLE
        )
    ...
    def test_deposit_with_savings_allocation(self):
        ...
        resp = self.client.post(
            url,
            {"amount": "1000", "allocate_to_savings": "300", "note": "initial"},
    ...
    def test_expense_from_spendable(self):
        ...
        resp = self.client.post(
            ...
            {"amount": "200", "allocation": "spendable", "merchant": "Cafe"},
            content_type="application/json",
        )
        self.assertEqual(resp.status_code, 200)
        data = resp.json()
    ...
    def test_expense_persists_category_and_food_items(self):
        ...
        item = Item.objects.create(
            category=category, subcategory=subcategory, name="Coffee"
        )
    ...
    def test_transfer_to_spendable(self):
        self.client.post(
            reverse("account-deposit", args=[self.acc.id]),
            {"amount": "500", "allocate_to_savings": "200"},
            content_type="application/json",
        )
        resp = self.client.post(
    ...
    def test_money_pools_track_allocation_totals(self):
        ...
        spendable_pool = MoneyPool.objects.get(
            account=self.acc,
            owner=owner,
            location=location,
            allocation_type=AllocationType.SPENDABLE,
        )
        savings_pool = MoneyPool.objects.get(
            account=self.acc,
            owner=owner,
            location=location,
